...
import { createBoxCastGeometry, expectShapeCastHit } from './helpers';

describe('PhysicsSystem -> collision-detection -> box shape casts', () => {
  it('Casts a box against a circle', () => {
    const query = createBoxCastGeometry(0, 0, 2, 2, 1, 0, 10);
  ...
  it('Casts a box diagonally against a circle', () => {
    const query = createBoxCastGeometry(0, 0, 2, 2, 1, 1, 10);
  ...
  it('Uses precomputed half extents for the support point', () => {
    const query = createBoxCastGeometry(0, 0, 4, 2, 1, 0, 10);
    const target = createBoxGeometry(7, 0, 2, 2);
  ...
  it('Returns false when a box cast misses', () => {
    const query = createBoxCastGeometry(0, 4, 2, 2, 1, 0, 10);
    const target = createBoxGeometry(5, 0, 2, 2);
