...
import { DebugVisualizerAPI } from './api';
...
export class DebugVisualizer extends SceneSystem {
  ...
  constructor(options: SceneSystemOptions) {
    ...
    this.mainActor = options.world.data.mainActor as Actor;
    ...
    const api = new DebugVisualizerAPI({
      debugActorMap: this.debugActorMap,
    });
    options.world.systemApi.register(api);
  }
  ...
  private handleActorRemove = ({ actor }: RemoveActorEvent): void => {
    this.debugActorMap.forEach((moduleActorMap) => {
      moduleActorMap.get(actor)?.remove();
    ...
    this.debugActorMap.forEach((moduleActorMap, module) => {
      moduleActorMap.forEach((debugActor, actor) => {
        if (debugActor === null) {
          ...
          this.mainActor.appendChild(debugActor);
