...
export const KeyPicker: FC<KeyPickerProps> = ({ value, onChange }) => {
  ...
  const handleKeyChange = useCallback<KeyboardEventHandler<HTMLInputElement>>(
    (event) => {
      if (editMode) {
  ...
  return (
    <Labelled label={t('components.keyboardControl.bind.key.title')}>
      <Space.Compact css={KeyPickerSpaceCompactCSS}>
        ...
        <Button
          icon={
            !editMode ? <Icon icon={<Pencil />} /> : <Icon icon={<Xmark />} />
          }
          onClick={handleClick}
        />
      </Space.Compact>
