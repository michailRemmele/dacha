...
import { Hand, Magnifier, HandPointUp, Person } from '@gravity-ui/icons';
...
const RADIO_THEME = {
  components: {
    Radio: { buttonColor: 'var(--ant-color-text-secondary)' },
  },
};
...
export const Toolbar: FC = () => {
  ...
  useEffect(() => {
    ...
    const handleToolUpdated = (): void => {
      ...
      const { name, features: currentFeatures } = toolActor.getComponent(Tool);

      setSelectedTool((prev) => (name !== prev ? name : prev));

      const featuresValues = Object.keys(currentFeatures).reduce(
        (acc, key) => {
      ...
      setToolFeatures((prev) =>
        !isEqual(featuresValues, prev) ? featuresValues : prev,
      );
    };
  ...
  return (
    <div className={styles.toolbar}>
      ...
      {features[selectedTool] && !disabled ? (
        <ToolFeatures features={toolFeatures} />
      ) : null}
    </div>
  );
};
