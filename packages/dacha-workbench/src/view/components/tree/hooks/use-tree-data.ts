...
export const useTreeData = (
  ...
  const parseTreeData = useCallback(
    (nodes: ExplorerDataNode[]): ExplorerDataNode[] =>
      nodes.map(({ children, className, ...node }) => ({
        ...node,
        children: children
          ? parseTreeData(children as ExplorerDataNode[])
          : undefined,
        className: `${className ?? ''} ${cutKeys.has(String(node.key)) ? 'tree-node_cut' : ''}`,
        'data-testid': 'explorer-tree-node',
      })),
    [cutKeys],
  );
