...
describe('resolveFieldInitialValue', () => {
  it('returns explicit initialValue when set', () => {
    expect(
      resolveFieldInitialValue({ name: 'x', type: 'number', initialValue: 5 }),
    ).toBe(5);
  });
  ...
  it('falls back to type default when initialValue is absent', () => {
    ...
    expect(resolveFieldInitialValue({ name: 'x', type: 'boolean' })).toBe(
      false,
    );
    expect(resolveFieldInitialValue({ name: 'x', type: 'color' })).toBe(
      '#ffffff',
    );
  ...
  it('resolves dependency chains in declaration order', () => {
    const fields: Field[] = [
      { name: 'a', type: 'boolean', initialValue: true },
      {
        ...
        type: 'boolean',
        initialValue: true,
        dependency: { name: 'a', value: true },
      },
      {
        name: 'c',
        type: 'number',
        initialValue: 7,
        dependency: { name: 'b', value: true },
      },
    ];
    ...
    const depFields: Field[] = [
      {
        ...
        type: 'string',
        initialValue: 'on',
        dependency: { name: 'nested.mode', value: 'on' },
      },
    ];
    expect(fillMissingFields({ nested: { mode: 'on' } }, depFields)).toEqual({
      nested: { mode: 'on' },
      flag: 'on',
    });
