...
import WidgetStore from '../../../components/diagrams/WidgetStore.astro';

The [field decorator](/game-code/inspector-fields/) generates a panel: one labeled input
...
const HealthWidget: FC<WidgetProps> = ({ path, fields, sections }) => {
  ...
  const [preset, setPreset] = useState<string | null>('grunt');
  ...
  const applyPreset = (name: string | null): void => {
    ...
    const brute = name === 'brute';

    dispatch(commands.setValue(path.concat('points'), brute ? 200 : 30));
    // `true` joins this write to the one above, so one undo takes back both
    dispatch(commands.setValue(path.concat('armor'), brute ? 5 : 0, true));
  };
...
Three rules are doing the work in that file.
...
that value changes, whoever changed it: your own write, another panel, an undo.

Writes go through a command, dispatched by the hook that owns the undo history:
...
The preset in the example writes this way, and the readout under it reads the same value back.
Pick a preset and the number of hits changes, because `useConfig` re-renders the widget when
the value it watches changes.
...
walks that stack backward and redo walks it forward. So a value written through a command
...
Passing `true` as the third argument attaches a write to the previous one instead of starting
a new entry. The two become a single step in the history:
...
Each labeled input has an unlabeled twin under the same name without the prefix, for when you
...
bundle, and the editor drops its widget and schema registries, loads the new bundle, and
redraws the inspector with it. That is the same rebuild [a generated class goes
through](/editor/generating-scripts/#after-create), and it clears the undo history for the
same reason.
